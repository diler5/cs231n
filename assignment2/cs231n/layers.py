...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        var = x.var(0) + eps
        std = np.sqrt(var)
        x_norm = (x - mean.reshape(-1, D)) / std.reshape(-1, D)
        out = gamma * x_norm + beta

        # Update computed distributions:
        ...
        x_norm = (x - running_mean.reshape(-1, D)) / np.sqrt(running_var.reshape(-1, D) + eps)
        out = gamma * x_norm + beta
...
def batchnorm_backward(dout, cache):
    ...
    [z, gamma, x, mean, std, var] = [cache[k] for k in ['z', 'gamma', 'x', 'mean', 'std', 'var']]
    dbeta = dout.sum(0)
    dgamma = np.sum(dout * cache['z'], 0)
    ...
    dvdu = -2 / N * np.sum(x - mean, 0) #[1xD]
    dx = dfdz * dzdx + \
      np.sum(dfdz * dzdu, 0) * dudx + \
      np.sum(dfdz * dzdv, 0) * (dvdx + dvdu * dudx) #[NxD]
...
def batchnorm_backward_alt(dout, cache):
    ...
    [z, gamma, x, mean, std, var] = [cache[k] for k in ['z', 'gamma', 'x', 'mean', 'std', 'var']]
    dbeta = dout.sum(0)
    dgamma = np.sum(dout * z, 0)

    N = dout.shape[0]
    dfdz = dout * gamma #[NxD]
    dfdz_sum = np.sum(dfdz, 0) #[1xD]
    dx = dfdz - dfdz_sum / N - np.sum(dfdz * z, 0) * z / N #[NxD]
    dx /= std
